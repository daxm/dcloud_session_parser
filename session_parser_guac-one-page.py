...
def build_document(doc, username, password, sessionline):
    ...
    """
    tmp_text = "Open your browser and go to {url}".format(url=sessionline['url'])
    p = doc.add_paragraph('Open your browser and go to ', style='List Bullet')
    p.add_run(sessionline['url']).bold = True
    p = doc.add_paragraph('Log in with username "', style='List Bullet')
    p.add_run('dcloud').bold = True
    p.add_run('" and password "')
    ...
    p.add_run('https://cisco.app.box.com/v/sda-test-drive').bold = True
    ...
    with open(fn, 'r') as f:
        for fline in f:
            ...
            if sessionline['usernames'] != 'Not Started' and sessionline['usernames'] != ' Usernames':
                ...
                url = 'http://' + str(sessionline['publicips']).strip('\n')
                sessionline['url'] = url
                password = sessionline['password']
